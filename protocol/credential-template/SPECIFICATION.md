...
1. For `ACTIVE` templates: exactly one of `issuer_key_id`, `issuer_certificate_chain_pem`, or `issuer_did` MUST be present unless `auto_generate_artifacts` is `true`.
2. `claims` MUST NOT be empty.
3. Claim names MUST be unique within a template.
4. `derived_from` MUST reference a valid `name` in the same template's `claims` array.
5. A `DRAFT` template MUST NOT be used in an active Flow or issuance trigger.
6. When `application_template_id` is set, the template MUST NOT be used for direct API issuance.
7. `compliance_profile_id` MUST reference an existing Compliance Profile.
